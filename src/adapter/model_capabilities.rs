...
impl ModelCapabilities {
    ...
    pub fn infer_token_limits(
        adapter_kind: AdapterKind,
        model_id: &str,
    ) -> (Option<u32>, Option<u32>) {
        provider_fallback!(Self::provider_token_limits, adapter_kind, model_id, Self::openai_infer_token_limits(model_id))
    }
    ...
    pub fn supports_streaming(adapter_kind: AdapterKind, model_id: &str) -> bool {
        provider_fallback!(Self::provider_supports_streaming, adapter_kind, model_id, Self::openai_supports_streaming(model_id))
    }

    /// Whether the model supports "function/tool" calls.
    pub fn supports_tool_calls(_adapter_kind: AdapterKind, _model_id: &str) -> bool {
        // 当前所有适配器均支持工具调用。如有差异，请在此调整。
        true
    }
    ...
    pub fn supports_json_mode(adapter_kind: AdapterKind, model_id: &str) -> bool {
        provider_fallback!(Self::provider_supports_json_mode, adapter_kind, model_id, Self::openai_supports_json_mode(model_id))
    }

    /// Whether the model supports reasoning effort control.
    pub fn supports_reasoning(adapter_kind: AdapterKind, model_id: &str) -> bool {
        provider_fallback!(Self::provider_supports_reasoning, adapter_kind, model_id, Self::openai_supports_reasoning(model_id))
    }

    /// Input modalities supported by the model.
    pub fn infer_input_modalities(
        adapter_kind: AdapterKind,
        model_id: &str,
    ) -> HashSet<Modality> {
        provider_fallback!(Self::provider_input_modalities, adapter_kind, model_id, Self::openai_infer_input_modalities(model_id))
    }

    /// Output modalities supported by the model.
    pub fn infer_output_modalities(
        adapter_kind: AdapterKind,
        model_id: &str,
    ) -> HashSet<Modality> {
        provider_fallback!(Self::provider_output_modalities, adapter_kind, model_id, Self::openai_infer_output_modalities(model_id))
    }

    /// Reasoning effort types supported by the model (if any).
    pub fn infer_reasoning_efforts(
        adapter_kind: AdapterKind,
        model_id: &str,
    ) -> Vec<ReasoningEffortType> {
        provider_fallback!(Self::provider_reasoning_efforts, adapter_kind, model_id, Self::openai_infer_reasoning_efforts(model_id))
    }
